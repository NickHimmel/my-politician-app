...
export const fetchFinances = (cid) => {
  return (dispatch, getState) => {
    ...
    const summary = fetch(`https://www.opensecrets.org/api/?method=candSummary&cid=${cid}&output=json&apikey=${token}`).then((response) => response.json());
    const contributors = fetch(`https://www.opensecrets.org/api/?method=candContrib&cid=${cid}&output=json&apikey=${token}`).then((response) => response.json());
    const industry = fetch(`https://www.opensecrets.org/api/?method=candIndustry&cid=${cid}&output=json&apikey=${token}`).then((response) => response.json());
    const sector = fetch(`https://www.opensecrets.org/api/?method=candSector&cid=${cid}&output=json&apikey=${token}`).then((response) => response.json());
